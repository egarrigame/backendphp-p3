...
@section('content')
    ...
    <form action="{{ route('incidencias.update', $incidencia) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select class="form-control @error('cliente_id') is-invalid @enderror" id="cliente_id" name="cliente_id" required>
                ...
                @foreach($clientes as $cliente)
                    <option value="{{ $cliente->id }}" {{ old('cliente_id', $incidencia->cliente_id) == $cliente->id ? 'selected' : '' }}>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control @error('descripcion') is-invalid @enderror" id="descripcion" name="descripcion" rows="3" required>{{ old('descripcion', $incidencia->descripcion) }}</textarea>
            @error('descripcion') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        <div class="mb-3">
            <label for="direccion" class="form-label">Dirección</label>
            <input type="text" class="form-control @error('direccion') is-invalid @enderror" id="direccion" name="direccion" value="{{ old('direccion', $incidencia->direccion) }}" required>
            @error('direccion') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        <div class="mb-3">
            <label for="fecha_servicio" class="form-label">Fecha del Servicio</label>
            <input type="datetime-local" class="form-control @error('fecha_servicio') is-invalid @enderror" id="fecha_servicio" name="fecha_servicio" value="{{ old('fecha_servicio', date('Y-m-d\TH:i', strtotime($incidencia->fecha_servicio))) }}" required>
        ...
        <div class="mb-3">
            ...
            <select class="form-control @error('tipo_urgencia') is-invalid @enderror" id="tipo_urgencia" name="tipo_urgencia" required>
                <option value="Estándar" {{ old('tipo_urgencia', $incidencia->tipo_urgencia) == 'Estándar' ? 'selected' : '' }}>Estándar</option>
        ...
        <div class="mb-3">
            ...
            <select class="form-control @error('estado') is-invalid @enderror" id="estado" name="estado" required>
                ...
                <option value="Cancelada" {{ old('estado', $incidencia->estado) == 'Cancelada' ? 'selected' : '' }}>Cancelada</option>
            </select>
